N =10000;w=40;P=0.001;g=9.81;a=0.2;phi=pi;
in=1:N;h=.00001;theta=0.0;
...
%H0 = H0 + eye(N);%Diagonal elements
    ...
    H0( 2 : ( N+1 ) : end ) = (1.0/h)*(+1i*a*w*P*sin(phi+theta)/2.0-(a*g)/2.0)*ones(1,N-1) ;
    H0( N+1 : ( N+1 ) : end ) =(1.0/h)*(-1i*a*w*P*sin(phi+theta)/2.0-(a*g)/2.0)*ones(1,N-1);
    
    H0(1,N) =(1.0/h)*(-1i*a*w*P*sin(phi+theta)/2.0-(a*g)/2.0);
    H0(N,1) = (1.0/h)*(+1i*a*w*P*sin(phi+theta)/2.0-(a*g)/2.0);
    
    
    H0( 3 : ( N+1 ) : end-N ) = (1.0/h)*(1.0/8.0)*a*a*w*w*cos(phi+theta)*cos(phi+theta)*ones(1,N-2) ;
    H0( 2*N+1 : ( N+1 ) : end ) = (1.0/h)*(1.0/8.0)*a*a*w*w*cos(phi+theta)*cos(phi+theta)*ones(1,N-2);
  ...
  %subplot(211);
  %plot(phi,(eig(H0)),'.')%eigenvalue vs pi plot
 % eig(H0)
